...
/** Curve fitting class based on the Simplex method in the io.binroot.regression.Minimizer class
 *
 *
 ...
 *	2012-01-30: Modified for external io.binroot.regression.Minimizer class and io.binroot.regression.UserFunction fit by Michael Schmid.
 ...
 *				by the simplex io.binroot.regression.Minimizer and improves convergence.	These parameters can be an offset and
...
public class CurveFitter implements UserFunction {
    ...
    public void doFit(int fitType, boolean showSettings) {
        ...
        //io.binroot.regression.IJ.log("special params: off="+offsetParam+(hasSlopeParam ? " slo=" : " fac=")+factorParam+" numPar="+numParams+" numRegressPar="+numRegressionParams);
...
//io.binroot.regression.IJ.log(io.binroot.regression.IJ.d2s(params[0],3,5)+","+io.binroot.regression.IJ.d2s(params[1],3,5)+": r="+io.binroot.regression.IJ.d2s(sumResidualsSqr,3,5)+Thread.currentThread().getName() );

        } else {	// handle simple linear dependencies by linear regression:
            //if(getIterations()<1){String s="minimizerPar:";for(int ii=0;ii<=numParams;ii++)s+=" ["+ii+"]:"+io.binroot.regression.IJ.d2s(params[ii],5,9);io.binroot.regression.IJ.log(s);}
