...
import io.binroot.regression.IJ;

import java.util.Random;
import java.util.Arrays;
import java.util.Vector;

/** io.binroot.regression.Minimizer based on Nelder-Mead simplex method (also known as polytope method),
 ...
 *
 * Versions:
 * Michael Schmid 2012-01-30: first version, based on previous io.binroot.regression.CurveFitter
...
public class Minimizer {
    ...
    // We can't set the function in the constructor because the io.binroot.regression.CurveFitter
     ...
     *                      the io.binroot.regression.UserFunction interface).
        ...
        for (int i=0; i<maxLoopCount; i++) {        // try several times, until we have twice the same result
            ...
            if (maxRestarts>0 && !useSingleThread) {  // set up 2nd thread to minimize
                ...
                final Thread thread = new Thread(
                        ...
                        }, "io.binroot.regression.Minimizer-1"
                );
     ...
     *  If displayStatusAndCheckEsc has been called before, the io.binroot.regression.Minimizer itself checks for the ESC key.
     */
    public void abort() {
        status = ABORTED;
    }
    ...
    private boolean initializeSimplex(double[][] simp, double[] paramVariations, Random random) {
        ...
        //io.binroot.regression.IJ.log("smallestR="+smallestRange+" largestR="+largestRange);
        ...
        for (int v=1; v<numVertices; v++) {
            ...
            do {                                //try finding a vertex that does not yield NaN
                ...
                } //else io.binroot.regression.IJ.log("no orthogonalization v#"+v);
                ...
                //io.binroot.regression.IJ.log("[0] var="+paramVariations[0]+"simp0="+simp[0][0]+" rand="+io.binroot.regression.IJ.d2s(nonZeroRandom,4)+" simpV="+simp[v][0]);
                evaluate(simp[v]);
